...
const ActivityRunner: React.FC = () => {
  ...
  const isTracingMode = topic === 'tracing';

  const recognitionRef = useRef<any>(null);

  // Initialize Speech Recognition
  useEffect(() => {
    // Only init speech if NOT in spelling or tracing mode
    if (isSpellingMode || isTracingMode) return;
  ...
  // Setup Tracing Mode Canvas
  useEffect(() => {
    if (isTracingMode && currentItem) {
      setHasTraced(false);
      setFeedback({ msg: '', type: 'neutral' });
  ...
  }, [currentIndex, isTracingMode, currentItem]);
  ...
  return (
    <div className="min-h-screen w-full flex flex-col bg-cream-50 overflow-hidden relative">
      ...
      <div className="flex-1 flex flex-col items-center justify-center p-4 pb-20">
        ...
        {/* Card or Question */}
        <div className={`relative w-full max-w-sm aspect-square bg-white rounded-3xl shadow-xl border-b-8 border-r-8 border-peach-400 flex flex-col items-center justify-center mb-6 transform transition-transform hover:scale-[1.02] ${isSpellingMode ? 'aspect-video h-48' : ''} ${isTracingMode ? 'aspect-square' : ''}`}>
          
          {isTracingMode ? (
            <div className="relative w-full h-full flex items-center justify-center">
              {/* Letter Outline to Trace */}
              <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
                <div className="text-[20rem] font-bold text-gray-200 select-none leading-none">
                  {currentItem.content}
                </div>
              </div>
              {/* Canvas for Drawing */}
            ...
            <div className="flex flex-col items-center">
               <div className="text-8xl animate-bounce-slight select-none">
                 {currentItem.example?.split(' ').pop() || '❓'}
               </div>
            </div>
          ) : (
            <>
              <div className="text-9xl font-bold text-midnight-900 animate-bounce-slight select-none">
                {currentItem.content}
              </div>
              {currentItem.example && (
        ...
        <div className="flex items-center justify-center gap-4 md:gap-8 w-full">
          ...
          {isTracingMode ? (
            <>
              ...
              >
                <div className="w-20 h-20 rounded-full bg-white border-4 border-orange-500 flex items-center justify-center shadow-lg group-active:scale-95 transition-transform">
                  <RotateCcw size={32} className="text-orange-500" />
                </div>
                <span className="font-bold text-midnight-900 text-sm md:text-base">Clear</span>
              </button>
              ...
              <button
                ...
                className="flex flex-col items-center gap-2 group"
              >
                <div className={`w-20 h-20 rounded-full flex items-center justify-center shadow-lg transition-transform ${
                  hasTraced 
                    ? 'bg-white border-4 border-green-500 group-active:scale-95' 
                    : 'bg-gray-200 border-4 border-gray-300 opacity-50 cursor-not-allowed'
                }`}>
                  <ArrowRight size={32} className={hasTraced ? 'text-green-500' : 'text-gray-400'} />
                </div>
                <span className="font-bold text-midnight-900 text-sm md:text-base">{t.next}</span>
              </button>
            </>
          ) : (
            <>
